...
public class UserService {
	...
	// return ContactModel.dao.find("select * from t_contact where uid=? and
	// status=0", uid);
	// }
	//
	...
	// * @return
	// */
	// public static ContactModel addOrUpdateContact(Integer id, String uid,
	...
	// * @return
	// */
	// public static Result<RegisterModel> register(AuthInfo authInfo,
	...
	//
	// // 请求医院接口 开始预约
	// String soap = service.register(authInfo, OperType.AddReg, null,
	// patientInfo, regInfo, regFrom);
	...
	// registerModel.setFetchEndTime(time.substring(0, 10) + ' ' +
	// time.substring(17, 22));
	//
	...
	// try {
	// return service.register(authInfo, OperType.DelReg, model.getSn(), info,
	// null, regFrom);
	// } catch (IOException e) {
	// e.printStackTrace();
	// }
	...
	// }
	//
	// public static List<NoticeModel> getNoticesFromCache(Integer page, Integer
	// pageSize) {
	// String key = StringHandler.joint('_', Const.Cache_Key_Notice, "all", page
	// != null ? page.toString() : "1",
	// pageSize != null ? pageSize.toString() : "10");
	// List<NoticeModel> list = CacheHandler.cache(Const.Cache_Name_request,
	// key);
	// if (list == null) {
	// String sql = "select id,title,updateTime,author,remark from t_notice
	// where status=0 order by sort desc,updateTime desc limit ?,?";
